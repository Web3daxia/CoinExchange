 ...
 * - 结合技术指标和资金流向进行交易
 */
@Component("FUND_MANAGEMENT_FUTURES_USDT")
public class FundManagementStrategy implements StrategyService {
    ...
    public void execute(Long userId, String pairName, String marketType, Map<String, Object> params) throws Exception {
        ...
        BigDecimal maxPositionRatio = getBigDecimalParam(params, "maxPositionRatio", new BigDecimal("0.3")); // 最大仓位比例
        ...
        boolean volumeIncrease = avgVolume.compareTo(BigDecimal.ZERO) > 0 && 
                                recentVolume.compareTo(avgVolume.multiply(new BigDecimal("1.2"))) > 0; // 成交量增加20%
        ...
        if (orderService != null) {
            BigDecimal baseAmount = new BigDecimal("100"); // TODO: 从账户余额和investmentRatio计算
            BigDecimal orderAmount = baseAmount.multiply(maxPositionRatio);
