 ...
 * 币本位永续合约使用-SWAP后缀，但需要指定币本位
 */
@Component
public class OKXCoinFuturesMarketDataAdapter implements CoinFuturesMarketDataAdapter {
    ...
    // OKX币本位永续合约API基础地址
    private static final String OKX_COIN_FUTURES_API_BASE = "https://www.okx.com/api/v5";

    @Override
    public String convertSymbol(String pairName) {
        // BTC/BTC -> BTC-USD-SWAP (OKX币本位永续合约使用-USD-SWAP后缀)
    ...
    public Map<String, Object> get24hrTicker(String symbol) {
        try {
            // OKX币本位永续合约24小时行情: GET /api/v5/market/ticker?instId=BTC-USD-SWAP
            // symbol已经是转换后的格式，直接使用
            String url = OKX_COIN_FUTURES_API_BASE + "/market/ticker?instId=" + symbol;
            String response = restTemplate.getForObject(url, String.class);
            Map<String, Object> data = objectMapper.readValue(response, new TypeReference<Map<String, Object>>() {});
            
            // OKX返回格式: {"code":"0","data":[{"instId":"BTC-USD-SWAP","last":"50000",...}],...}
            ...
            result.put("amount24h", new BigDecimal(ticker.get("volCcy24h").toString()));
            ...
            throw new RuntimeException("获取OKX币本位永续合约24小时统计数据失败: " + e.getMessage(), e);
        }
    }
    ...
    public Map<String, Object> getDepth(String symbol, Integer limit) {
        try {
            // OKX币本位永续合约订单簿: GET /api/v5/market/books?instId=BTC-USD-SWAP
            String url = OKX_COIN_FUTURES_API_BASE + "/market/books?instId=" + symbol + "&sz=" + limit;
            String response = restTemplate.getForObject(url, String.class);
            Map<String, Object> data = objectMapper.readValue(response, new TypeReference<Map<String, Object>>() {});
            ...
            throw new RuntimeException("获取OKX币本位永续合约市场深度失败: " + e.getMessage(), e);
        }
    }
    ...
    public List<Map<String, Object>> getKlineData(String symbol, String interval, Integer limit) {
        try {
            ...
            // OKX币本位永续合约K线数据: GET /api/v5/market/candles?instId=BTC-USD-SWAP
            String url = OKX_COIN_FUTURES_API_BASE + "/market/candles?instId=" + symbol 
            ...
            throw new RuntimeException("获取OKX币本位永续合约K线数据失败: " + e.getMessage(), e);
        }
    }
    ...
    public Map<String, Object> getMarkPriceAndIndexPrice(String symbol) {
        try {
            // OKX币本位永续合约标记价格: GET /api/v5/public/mark-price?instId=BTC-USD-SWAP
            String url = OKX_COIN_FUTURES_API_BASE + "/public/mark-price?instId=" + symbol;
            String response = restTemplate.getForObject(url, String.class);
            Map<String, Object> data = objectMapper.readValue(response, new TypeReference<Map<String, Object>>() {});
            
            // OKX返回格式: {"code":"0","data":[{"instId":"BTC-USD-SWAP","markPx":"50000","idxPx":"49999",...}],...}
            List<Map<String, Object>> dataList = (List<Map<String, Object>>) data.get("data");
            if (dataList == null || dataList.isEmpty()) {
                throw new RuntimeException("OKX API返回数据为空");
            }
            
            Map<String, Object> markData = dataList.get(0);
            
            Map<String, Object> result = new HashMap<>();
            result.put("markPrice", new BigDecimal(markData.get("markPx").toString()));
            result.put("indexPrice", new BigDecimal(markData.get("idxPx").toString()));
            return result;
        } catch (Exception e) {
            throw new RuntimeException("获取OKX币本位永续合约标记价格和指数价格失败: " + e.getMessage(), e);
        }
    }
    ...
    public BigDecimal getFundingRate(String symbol) {
        try {
            // OKX币本位永续合约资金费率: GET /api/v5/public/funding-rate?instId=BTC-USD-SWAP
            String url = OKX_COIN_FUTURES_API_BASE + "/public/funding-rate?instId=" + symbol;
            String response = restTemplate.getForObject(url, String.class);
            Map<String, Object> data = objectMapper.readValue(response, new TypeReference<Map<String, Object>>() {});
            
            // OKX返回格式: {"code":"0","data":[{"instId":"BTC-USD-SWAP","fundingRate":"0.0001",...}],...}
            List<Map<String, Object>> dataList = (List<Map<String, Object>>) data.get("data");
            if (dataList == null || dataList.isEmpty()) {
            ...
            if (fundingData.get("fundingRate") != null) {
                return new BigDecimal(fundingData.get("fundingRate").toString());
            }
            return BigDecimal.ZERO;
        } catch (Exception e) {
            throw new RuntimeException("获取OKX币本位永续合约资金费率失败: " + e.getMessage(), e);
        }
    }
