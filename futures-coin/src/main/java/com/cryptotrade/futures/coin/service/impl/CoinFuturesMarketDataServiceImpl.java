...
@Service
public class CoinFuturesMarketDataServiceImpl implements CoinFuturesMarketDataService {
    ...
    @Autowired
    private com.cryptotrade.futures.coin.service.impl.BinanceCoinFuturesMarketDataAdapter binanceAdapter;

    @Autowired
    private com.cryptotrade.futures.coin.service.impl.OKXCoinFuturesMarketDataAdapter okxAdapter;

    @Autowired
    private com.cryptotrade.futures.coin.service.impl.GateCoinFuturesMarketDataAdapter gateAdapter;

    @Autowired
    private com.cryptotrade.futures.coin.service.impl.HuobiCoinFuturesMarketDataAdapter huobiAdapter;

    private static final String MARKET_DATA_PREFIX = "coin-futures:market:data:";
    private static final String CURRENT_DATA_SOURCE_KEY = "coin-futures:market:data:current_source";

    @Override
    public CoinFuturesMarketDataResponse getMarketData(String pairName) {
        CoinFuturesContract contract = coinFuturesContractRepository
                .findByPairNameAndContractType(pairName, "COIN_MARGINED")
                .orElseThrow(() -> new RuntimeException("币本位合约不存在"));

        // 尝试从实时API获取最新数据
        try {
            CoinFuturesMarketDataAdapter adapter = getCurrentAdapter();
            ...
            CoinFuturesMarketDataResponse response = new CoinFuturesMarketDataResponse();
            ...
            response.setBids((List<Map<String, Object>>) depth.get("bids"));
            response.setAsks((List<Map<String, Object>>) depth.get("asks"));
    ...
    public MarketDepthResponse getMarketDepthEnhanced(MarketDepthRequest request) {
        ...
        // 获取当前价格
        CoinFuturesMarketDataResponse marketData = getMarketData(pair);
    ...
    public void syncMarketData(String pairName) {
        ...
        try {
            ...
            coinFuturesContractRepository.save(contract);
        } catch (Exception e) {
    ...
    public void switchDataSource(String dataSource) {
        ...
        // 更新配置（与现货交易共享配置表，但使用不同的Redis key区分）
     ...
     * 获取当前数据源对应的适配器
     */
    private CoinFuturesMarketDataAdapter getCurrentAdapter() {
