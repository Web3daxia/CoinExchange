...
/**
 * 币本位永续合约仓位管理实体（持仓管理/合约资产管理）
 * 与U本位结构相同，但存储在独立表中
 */
@Entity
@Table(name = "coin_futures_position_management")
@Data
@EntityListeners(AuditingEntityListener.class)
public class CoinFuturesPositionManagement {
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long id;

    @Column(name = "position_id", unique = true, nullable = false)
    private Long positionId; // 仓位ID（关联coin_futures_positions表）
    ...
    private String accountType = "FUTURES_COIN"; // 合约账户类型
