 ...
 */
@Component
public class HuobiFuturesMarketDataAdapter implements FuturesMarketDataAdapter {
    ...
    // 火币永续合约API基础地址
    private static final String HUOBI_FUTURES_API_BASE = "https://api.hbdm.com";

    @Override
    public String convertSymbol(String pairName) {
        // BTC/USDT -> BTC-USDT (火币永续合约使用连字符，合约代码为BTC-USDT)
    ...
    public Map<String, Object> get24hrTicker(String symbol) {
        try {
            // 火币永续合约24小时行情: GET /linear-swap-ex/market/detail/merged?contract_code=BTC-USDT
            String huobiSymbol = convertSymbol(symbol);
            String url = HUOBI_FUTURES_API_BASE + "/linear-swap-ex/market/detail/merged?contract_code=" + huobiSymbol;
            String response = restTemplate.getForObject(url, String.class);
            Map<String, Object> data = objectMapper.readValue(response, new TypeReference<Map<String, Object>>() {});
            
            // 火币返回格式: {"status":"ok","ch":"market.BTC-USDT.detail.merged","ts":1234567890,"tick":{...}}
            ...
            // 获取24小时统计数据
            String tickerUrl = HUOBI_FUTURES_API_BASE + "/linear-swap-ex/market/detail?contract_code=" + huobiSymbol;
            ...
            throw new RuntimeException("获取火币永续合约24小时统计数据失败: " + e.getMessage(), e);
        }
    }
    ...
    public Map<String, Object> getDepth(String symbol, Integer limit) {
        try {
            String huobiSymbol = convertSymbol(symbol);
            // 火币永续合约订单簿: GET /linear-swap-ex/market/depth?contract_code=BTC-USDT&type=step0
            String url = HUOBI_FUTURES_API_BASE + "/linear-swap-ex/market/depth?contract_code=" + huobiSymbol + "&type=step0";
            String response = restTemplate.getForObject(url, String.class);
            Map<String, Object> data = objectMapper.readValue(response, new TypeReference<Map<String, Object>>() {});
            
            // 火币返回格式: {"status":"ok","ch":"market.BTC-USDT.depth.step0","ts":1234567890,"tick":{"bids":[[price,quantity],...],"asks":[[price,quantity],...]}}
            String status = (String) data.get("status");
            if (!"ok".equals(status)) {
                throw new RuntimeException("火币API返回状态错误: " + status);
            }
            ...
            throw new RuntimeException("获取火币永续合约市场深度失败: " + e.getMessage(), e);
        }
    }
    ...
    public List<Map<String, Object>> getKlineData(String symbol, String interval, Integer limit) {
        try {
            ...
            // 火币永续合约K线数据: GET /linear-swap-ex/market/history/kline?contract_code=BTC-USDT&period=1min&size=100
            String url = HUOBI_FUTURES_API_BASE + "/linear-swap-ex/market/history/kline?contract_code=" + huobiSymbol 
                    + "&period=" + huobiInterval + "&size=" + limit;
            String response = restTemplate.getForObject(url, String.class);
            Map<String, Object> data = objectMapper.readValue(response, new TypeReference<Map<String, Object>>() {});
            
            // 火币返回格式: {"status":"ok","ch":"market.BTC-USDT.kline.1min","ts":1234567890,"data":[{...}]}
            String status = (String) data.get("status");
            if (!"ok".equals(status)) {
                throw new RuntimeException("火币API返回状态错误: " + status);
            }
            ...
            throw new RuntimeException("获取火币永续合约K线数据失败: " + e.getMessage(), e);
        }
    }
    ...
    public Map<String, Object> getMarkPriceAndIndexPrice(String symbol) {
        try {
            String huobiSymbol = convertSymbol(symbol);
            // 火币永续合约标记价格: GET /linear-swap-ex/market/detail/merged?contract_code=BTC-USDT
            // 或者使用: GET /linear-swap-ex/market/batch_merged?contract_code=BTC-USDT
            String url = HUOBI_FUTURES_API_BASE + "/linear-swap-ex/market/detail/merged?contract_code=" + huobiSymbol;
            String response = restTemplate.getForObject(url, String.class);
            Map<String, Object> data = objectMapper.readValue(response, new TypeReference<Map<String, Object>>() {});
            ...
            throw new RuntimeException("获取火币永续合约标记价格和指数价格失败: " + e.getMessage(), e);
        }
    }
    ...
    public BigDecimal getFundingRate(String symbol) {
        try {
            String huobiSymbol = convertSymbol(symbol);
            // 火币永续合约资金费率: GET /linear-swap-ex/market/batch_funding_rate?contract_code=BTC-USDT
            String url = HUOBI_FUTURES_API_BASE + "/linear-swap-ex/market/batch_funding_rate?contract_code=" + huobiSymbol;
            String response = restTemplate.getForObject(url, String.class);
            Map<String, Object> data = objectMapper.readValue(response, new TypeReference<Map<String, Object>>() {});
            
            // 火币返回格式: {"status":"ok","data":[{"contract_code":"BTC-USDT","funding_rate":"0.0001",...}],...}
            String status = (String) data.get("status");
            if (!"ok".equals(status)) {
            ...
            throw new RuntimeException("获取火币永续合约资金费率失败: " + e.getMessage(), e);
        }
    }
