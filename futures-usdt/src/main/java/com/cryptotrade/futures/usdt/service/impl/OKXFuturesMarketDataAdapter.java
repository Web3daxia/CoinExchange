 ...
 */
@Component
public class OKXFuturesMarketDataAdapter implements FuturesMarketDataAdapter {
    ...
    // OKX永续合约API基础地址
    private static final String OKX_FUTURES_API_BASE = "https://www.okx.com/api/v5";

    @Override
    public String convertSymbol(String pairName) {
        // BTC/USDT -> BTC-USDT-SWAP (OKX永续合约使用-SWAP后缀)
        return pairName.replace("/", "-") + "-SWAP";
    }

    @Override
    public Map<String, Object> get24hrTicker(String symbol) {
        try {
            // OKX永续合约24小时行情: GET /api/v5/market/ticker?instId=BTC-USDT-SWAP
            String okxSymbol = convertSymbol(symbol);
            String url = OKX_FUTURES_API_BASE + "/market/ticker?instId=" + okxSymbol;
            String response = restTemplate.getForObject(url, String.class);
            Map<String, Object> data = objectMapper.readValue(response, new TypeReference<Map<String, Object>>() {});
            
            // OKX返回格式: {"code":"0","data":[{"instId":"BTC-USDT-SWAP","last":"50000",...}],...}
            ...
            throw new RuntimeException("获取OKX永续合约24小时统计数据失败: " + e.getMessage(), e);
        }
    }
    ...
    public Map<String, Object> getDepth(String symbol, Integer limit) {
        try {
            String okxSymbol = convertSymbol(symbol);
            // OKX永续合约订单簿: GET /api/v5/market/books?instId=BTC-USDT-SWAP
            String url = OKX_FUTURES_API_BASE + "/market/books?instId=" + okxSymbol + "&sz=" + limit;
            String response = restTemplate.getForObject(url, String.class);
            Map<String, Object> data = objectMapper.readValue(response, new TypeReference<Map<String, Object>>() {});
            ...
            throw new RuntimeException("获取OKX永续合约市场深度失败: " + e.getMessage(), e);
        }
    }
    ...
    public List<Map<String, Object>> getKlineData(String symbol, String interval, Integer limit) {
        try {
            ...
            // OKX永续合约K线数据: GET /api/v5/market/candles?instId=BTC-USDT-SWAP
            String url = OKX_FUTURES_API_BASE + "/market/candles?instId=" + okxSymbol 
            ...
            throw new RuntimeException("获取OKX永续合约K线数据失败: " + e.getMessage(), e);
        }
    }
    ...
    public Map<String, Object> getMarkPriceAndIndexPrice(String symbol) {
        try {
            String okxSymbol = convertSymbol(symbol);
            // OKX永续合约标记价格: GET /api/v5/public/mark-price?instId=BTC-USDT-SWAP
            String url = OKX_FUTURES_API_BASE + "/public/mark-price?instId=" + okxSymbol;
            String response = restTemplate.getForObject(url, String.class);
            Map<String, Object> data = objectMapper.readValue(response, new TypeReference<Map<String, Object>>() {});
            
            // OKX返回格式: {"code":"0","data":[{"instId":"BTC-USDT-SWAP","markPx":"50000","idxPx":"49999",...}],...}
            List<Map<String, Object>> dataList = (List<Map<String, Object>>) data.get("data");
            if (dataList == null || dataList.isEmpty()) {
                throw new RuntimeException("OKX API返回数据为空");
            }
            ...
            throw new RuntimeException("获取OKX永续合约标记价格和指数价格失败: " + e.getMessage(), e);
        }
    }
    ...
    public BigDecimal getFundingRate(String symbol) {
        try {
            String okxSymbol = convertSymbol(symbol);
            // OKX永续合约资金费率: GET /api/v5/public/funding-rate?instId=BTC-USDT-SWAP
            String url = OKX_FUTURES_API_BASE + "/public/funding-rate?instId=" + okxSymbol;
            String response = restTemplate.getForObject(url, String.class);
            Map<String, Object> data = objectMapper.readValue(response, new TypeReference<Map<String, Object>>() {});
            
            // OKX返回格式: {"code":"0","data":[{"instId":"BTC-USDT-SWAP","fundingRate":"0.0001",...}],...}
            List<Map<String, Object>> dataList = (List<Map<String, Object>>) data.get("data");
            if (dataList == null || dataList.isEmpty()) {
            ...
            throw new RuntimeException("获取OKX永续合约资金费率失败: " + e.getMessage(), e);
        }
    }
