 ...
 */
@Entity
@Table(name = "usdt_futures_trading_pairs")
@Data
@EntityListeners(AuditingEntityListener.class)
public class UsdtFuturesTradingPair {
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long id;

    @Column(name = "pair_name", unique = true, nullable = false)
    private String pairName; // 交易对名称，如: LINK/USDT
    ...
    private String contractName; // 合约名称，如: LINK永续合约

    @Column(name = "base_currency", nullable = false)
    private String baseCurrency; // 基础币种，如: LINK

    @Column(name = "quote_currency", nullable = false)
    private String quoteCurrency; // 基础量价单位，如: USDT
    ...
    private String leverageType; // 杠杆倍数类型: DISCRETE（离散倍数）, CONTINUOUS（连续倍数）

    @Column(name = "leverage_multipliers", length = 500)
    private String leverageMultipliers; // 杠杆倍数，如: 5,10,20,50,100（JSON数组或逗号分隔）
    ...
    @Column(name = "funding_rate_settlement_cycle", nullable = false)
    private Integer fundingRateSettlementCycle; // 资金费率结算周期（小时）
    ...
    private String slippageType = "PERCENTAGE"; // 滑点类型: PERCENTAGE（百分比）, FIXED（固定值）
    ...
    private String contractType = "USDT_MARGINED"; // 合约类型: USDT_MARGINED（U本位）, COIN_MARGINED（币本位）
