 ...
 */
@Entity
@Table(name = "control_panel_bot_configs")
@Data
@EntityListeners(AuditingEntityListener.class)
public class ControlPanelBotConfig {
    @Id
    ...
    // 二十四小时基础信息
    @Column(name = "daily_high", precision = 20, scale = 8)
    private BigDecimal dailyHigh; // 二十四小时最高价

    @Column(name = "daily_low", precision = 20, scale = 8)
    private BigDecimal dailyLow; // 二十四小时最低价

    @Column(name = "daily_volume", precision = 20, scale = 8)
    private BigDecimal dailyVolume; // 二十四小时交易量

    @Column(name = "daily_amount", precision = 20, scale = 8)
