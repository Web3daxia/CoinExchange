...
public class RegisterRequest {
    @ApiModelProperty(value = "注册类型: PHONE, EMAIL, PRIVATE_KEY", required = true, example = "PHONE")
    ...
    @ApiModelProperty(value = "密码（手机/邮箱注册时必填）", example = "Password123!")
    @Size(min = 8, max = 20, message = "密码长度必须在8-20位之间")
    @Pattern(regexp = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@$!%*?&]{8,}$",
            message = "密码必须包含大小写字母、数字和特殊字符")
    private String password;

    @ApiModelProperty(value = "私钥（私钥注册时必填）")
